...
encodeConstructed c@(Start _) children =
 ...
 where
  (clen, events) = encodeList children
...
getBoolean isDer s =
  if B.length s == 1
    then case B.head s of
      ...
      _    -> if isDer
        then Left $ PolicyFailed "DER" "boolean value not canonical"
        else Right (Boolean True)
    else
      Left $ TypeDecodingFailed "boolean: length not within bound"
...
getDoubleRaw s@(B.unsafeHead -> h)
  ...
  | otherwise = do
      ...
      let (_, exp'') = intOfBytes $
            B.unsafeTake (fromIntegral expLength) $ B.unsafeDrop 1 s
...
getBitString s =
  ...
  let toSkip' = if toSkip >= 48 && toSkip <= 48 + 7
        then toSkip - fromIntegral ( ord '0')
        else toSkip in
  let xs = B.tail s in
  if toSkip' >= 0 && toSkip' <= 7
    then Right $ BitString $ toBitArray xs (fromIntegral toSkip')
    else Left $ TypeDecodingFailed
...
getOID s = case B.unpack s of
  ...
  (x : xs) -> Right $ OID
    ( fromIntegral (x `div` 40)
    : fromIntegral (x `mod` 40)
    : groupOID xs
    )
 where
  ...
  spanSubOIDbound [] = ([], [])
  spanSubOIDbound (a:as) = if testBit a 7
    then (clearBit a 7 : ys, zs)
    else ([a], as)
   where
    (ys, zs) = spanSubOIDbound as
...
getTime timeType bs
 ...
 where
  ...
  format
    | timeType == TimeGeneralized = 'Y':'Y':baseFormat
  ...
  parseTimezoneFormat transform s
      ...
      | otherwise      = Nothing
   where
    toTz z = let (h,m) = z `divMod` 100
             in  TimezoneOffset $ transform (h * 60 + m)
  ...
  decodingError reason = Left $ TypeDecodingFailed
    ("time format invalid for " ++ show timeType ++ " : " ++ reason)
